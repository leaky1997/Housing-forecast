...
def transfer_to_np1(filename = 'soochow456.csv',scaler=False):
    ...
    datapart1 = data.iloc[:,-6:]
    datapart2 = data.iloc[:,-10]
    datapart = pd.concat([datapart1,datapart2],axis=1) 
    datapart = datapart[:-1]
    x = np.array(datapart)
...
#    for method in ['svm','tree','linear','knn','RFR','Adaboost','GBR','Bag','ETR','MLP']:
    ...
    x, y, x_scaler,y_scaler= transfer_to_np1(dataset1,scaler=True)
...
#    
    ...
    plot(val_targets,y_pred,str(score1)+method)
    plot(y_test_real,y_pred_real,str(score2)+method)
